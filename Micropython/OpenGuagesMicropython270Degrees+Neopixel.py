...
servorange = 116  #Edit for range of servo - 116 equates to approx 270 degrees with Open Gauges Gear Ratio
datarange = 60 #Data Range - in our case 60 for 0-60 MPH
   ...
   # if len(datalist) < 2:
    ...
    if datalist[0] == datalist[-1]:
        ...
        while datalist[0] >  datalist[-1]:
            ...
            servo(datalist[0])
            pixels.set_pixel(int(datalist[0]/pixelcal), (BLUE))
            pixels.set_pixel(int(datalist[0]/pixelcal)+1, (WHITE))
            pixels.show()
            sleep(servospeed)
